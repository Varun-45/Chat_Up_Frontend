...
const ENDPOINT = "https://chat-up-api.onrender.com"
...
const SingleChat = ({ fetch, setfetch }) => {
    ...
    const sendmsg = async (event) => {
        if (event.key === "Enter" && newMsg) {
            ...
            try {
                ...
                setnewMsg("");
                const { data } = await axios.post("https://chat-up-api.onrender.com/api/message", {
                    content: newMsg,
    ...
    const fetchmsg = async (event) => {
        ...
        try {
            ...
            setloading(true)
            const { data } = await axios.get(`https://chat-up-api.onrender.com/api/message/${selectedChat._id}`,
                config)
            console.log(data)
            setmessages(data)
            setloading(false)
